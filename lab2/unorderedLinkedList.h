...
void deleteSmallestNode() override {
    if (head == nullptr) {
        std::cout << "List is empty. Cannot delete the smallest node." << std::endl;
        return;
    }

    nodeType<Type>* current = head;
    nodeType<Type>* prev = nullptr;
    nodeType<Type>* minNode = head;
    Type minData = head->info;

    while (current != nullptr) {
        if (current->info < minData) {
            minData = current->info;
            minNode = current;
        }
        prev = current;
        current = current->link;
    ...
    if (prev == nullptr) {
        head = minNode->link; // Update head to the next node.
    } else {
        prev->link = minNode->link;
    }
...
void deleteAllOccurrences(Type info) override {
    if (head == nullptr) {
        std::cout << "List is empty. No occurrences to delete." << std::endl;
        return;
    }
